...
>
  {#if !googleAds}
    <div class="flex flex-col w-full text-branco {textContainerClasses}">
    ...
    <fieldset class="flex flex-col gap-3 items-center justify-center w-full">
      <label class="flex flex-col gap-1 w-full">
        <span class="nlf-paragrafo font-bold text-branco"
        ...
        >
          ...
          <option value="atendimento comercial"
            >Atendimento Comercial (planos, velocidades, valores)</option
          >
          <option value="atendimento ao cliente"
            >Atendimento ao Cliente (financeiro, suporte, etc)</option
          >
        </select>
      </label>
    </fieldset>
  ...
  {:else}
    <BotaoWpp
      mensagem={`Olá! Vim do Google Ads e gostaria de conversar sobre os planos. [NÃO EXCLUA]`}
      titulo="Falar com o time comercial"
      classes="z-[1000]"
    />
  {/if}
</div>
