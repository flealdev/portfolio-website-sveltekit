<script>
  ...
  import MenuMobile25anosModal from "../--mobile-25anos-modal/menu--mobile--25anos-modal.svelte";
  import MenuItem from "../__item/--25anos/menu__item--25anos.svelte";
...
<button
  class="p-[10px] xl:hidden xl:justify-center cursor-pointer xl:w-full max-xl:bg-branco-1-principal-claro font-bold items-center rounded-[5px] text-azul-4-principal flex gap-[10px] {order}"
  onclick={() => {
    const menuItems = document.getElementById("menu-items");
    const arrowMenu = document.getElementById("arrow-menu");
  ...
  >
    <g clip-path="url(#clip0_635_1555)">
      <path
        ...
        class="max-xl:fill-azul-4-principal xl:fill-branco"
      />
    </g>
    <defs>
...
<div
  id="menu-items"
  class="max-md:flex-col max-xl:w-full duration-500 max-md:py-5 max-md:hidden md:flex md:items-center md:justify-between transition-all items-end text-branco-1-principal-claro justify-center max-lg:gap-5 lg:gap-[25px] {classes} {order}"
>
  <div
    class="flex items-center flex-wrap text-branco-1-principal-claro justify-end max-lg:gap-[10px] lg:gap-[25px]"
  >
  ...
  >
    ...
    <p class="nlf-item-com-icone text-branco font-bold uppercase">Ver mais</p>
  </button>
</div>
